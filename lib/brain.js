...
function siteContext(s) {
  const lines = [];
  lines.push(`SITE ${s.id} (${s.name}, ${s.type}) — status: ${s.status.toUpperCase()}`);
  if (s.crew_en_route)
...
{
  "site_id": "<exact SITE id it concerns, or null if unclear/general>",
...
export async function brainProcess(state, who, text) {
  ...
  state.feed.unshift({ ts: Date.now(), who, site_id: out.site_id || null, text, reply: out.reply || "Logged." });
