...
// priority tier + barrels/day — drives triage: a day of downtime on an A well
// dwarfs the cost of any truck roll.
const PRIORITY = {
  "42-17": ["A", 310],
  "38-2": ["B", 140],
  "HARTLEY-23-4": ["B", 95],
  "DOVE-CREEK-8-11": ["C", 40],
  "TXL-7": ["A", null], // battery gathers half the field's production
  "44-9": ["C", 35],
  "31-5": ["A", 260],
  "PECOS-PAD-A": ["C", null],
  "27-12": ["B", 110],
  "53-1": ["C", 55],
  "MIDKIFF-SWD": ["A", null], // field stops if disposal stops
  "61-3": ["B", 150],
};
...
export function seedState() {
  ...
  const state = {
    ...
    updated: now,
    crews: [
      { team: "PUMPER", members: ["Luis G.", "Carlos M."] },
      { team: "MECHANIC", members: ["Tasha B.", "Danny O."] },
      { team: "ELECTRIC (I&E)", members: ["Mike R."] },
      { team: "BATTERY", members: ["Priya S."] },
      { team: "SCADA/IT", members: ["Jordan K."] },
      { team: "HSE", members: ["Sam W."] },
    ],
    sites: [
      {
        id: "42-17",
      ...
      },
      {
        ...
        status: "attention",
        crew_en_route: [{ name: "Luis G.", team: "PUMPER", eta: "today ~14:00" }],
        open_items: [
      ...
      },
      {
        ...
        status: "down",
        crew_en_route: [{ name: "Tasha B.", team: "MECHANIC", eta: "tomorrow 08:00 (crane truck)" }],
        open_items: [
    ...
    metrics: { trips_avoided: 0, items_closed: 0 }
  };
  for (const s of state.sites) {
    const p = PRIORITY[s.id] || ["C", null];
    s.priority = p[0];
    s.bpd = p[1];
  }
  return state;
